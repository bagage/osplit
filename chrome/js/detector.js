/**
  ...
  */
  ...
  if (gecoDataUrl) {
    chrome.runtime.sendMessage({cmd:'gecodetected', val:gecoDataUrl});
  } else {
    ...
    if (scriptTag) {
      window.addEventListener("message", function(event) {
            ...
            if(msg.cmd==='jsonResponse'){
              ...
              chrome.runtime.sendMessage({cmd:'gecodetected', old:true});
            }
          });
      window.postMessage({cmd:'getJson'}, '*');
    } else if (regex.test(document.body.innerText)) {
      chrome.runtime.sendMessage({cmd:'oedetected'});
    }
  }
