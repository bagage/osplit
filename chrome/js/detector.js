/**
  ...
  */
  ...
  if (gecoDataUrl) {
    browser.runtime.sendMessage({cmd:'gecodetected', val:gecoDataUrl});
  } else {
    ...
    if (scriptTag) {
      window.addEventListener("message", function(event) {
            ...
            if(msg.cmd==='jsonResponse'){
              ...
              browser.runtime.sendMessage({cmd:'gecodetected', old:true});
            }
          });
      window.postMessage({cmd:'getJson'}, '*');
    } else if (regex.test(document.body.innerText)) {
      browser.runtime.sendMessage({cmd:'oedetected'});
    }
  }
